 ...
 * Time: 9:45
 */
public class DeleteChatParticipantAction extends BaseAction{
    private long uid;
    private long chat_id;

    public DeleteChatParticipantAction(ResultReceiver mReceiver, Bundle args) {
    ...
    protected JSONObject doInBackground(Void... voids) {
        ...
        return  VKApi.deleteChatParticipant(chat_id, uid);
        ...
        if (json!= null){
            ...
            try {
                if (json.has("response") && json.getInt("response") == 1){
                    sendResult(VKService.Result.PARTICIPANT_DELETED);
                } else {
                    sendResult(VKService.Result.PARTICIPANT_NOT_DELETED);
                }
